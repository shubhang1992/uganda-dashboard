...
3. **`src/subscriber-dashboard/pages/ProfilePage.jsx`** — uses `useState(sub?.x || '')` for name/email/phoneDigits without a hydration `useEffect`. Form renders empty for users until they retype. Same class as the agent/branch identity fix from the prior session. See the comment in `e2e/specs/flows/subscriber-edit-profile.spec.ts`.
...
⏳ **Future** — Vision: `/qa explore <role>` (computer-use exploratory pass) and `/qa screenshot-review` (multimodal Read on captured PNGs). Fixing the agent-onboard AML-step hang. Wiring the UI-mock `CreateBranch` panel to `useCreateBranch`. Hydration fix for `subscriber-dashboard/pages/ProfilePage.jsx`.
