...
> The box reflects the **full live state of the new Singapore DB** (`ilkhfnoyxlxwqadebnkp`, `ap-southeast-1`, cutover **2026-06-05**), with every migration `0001`–`0062` applied — so the counts already include the employer family + the `employer_invites` invite table (`0047`), the settlement/notification tables (`0030`/`0031`), the idempotency ledgers (`settlement_uploads` `0032`, `contribution_run_uploads` `0034`, `subscriber_signup_uploads` `0042`), the `0041` commission-aggregate RPCs, the **admin** role's RLS clones + create/overview/settlement RPCs (`0049`–`0051`), the `0052`–`0057` audit-remediation RPCs, the platform-scope + employer-activity rollups (`0058`/`0059`), the deactivate/reactivate stack (`0060`/`0061`), and the contribution-model-v2 RPCs (`0062`). Verified counts (audit §0/§1b, verified-live): 28 tables · 2 ENUMs · 5 triggers · **40 functions** (29 SECURITY DEFINER + 11 INVOKER, including the user-facing RPCs, private `_`-prefixed helpers, and trigger functions) — TODO: this `pg_proc` figure predates `0058`–`0062`, which added `get_employer_geo_rollup` (`0058`), `get_employer_activity_rollup` (`0059`), `set_distributor_status`/`set_employer_status`/`block_inactive_employer_subscriber`/`block_inactive_employer_run` (`0060`), `block_inactive_employer_subscriber_update` (`0061`), and `update_employer_member_compensation` (`0062`) — re-verify against live `pg_proc` before relying on it · **~90 RLS policies** (the `0049` admin clones stacked one `*_select_admin` policy onto each readable table). The table count of **28 is unchanged** — `0058`–`0062` added no tables (verified: no `CREATE TABLE` in those migrations).
...
Files are zero-padded, monotonically increasing. **0019 was historically absent** — it was a remote-only raw-psql hotfix for the metrics-rollup `app_role` bug, never committed at the time (the canonical body fix landed as 0020). It has since been **backfilled as `0019_fix_metrics_rollup_app_role.sql`** (a defensive, idempotent ACL-only migration capturing the original hotfix), so the tree now holds a contiguous `0001`–`0062`. The new Singapore DB records it in the ledger as `0019`. See `0020_entity_metrics_rollup_v3.sql:3–5` for the supersession history.

### `.down.sql` partners

Newer migrations ship a `.down.sql` partner alongside the forward file (`0016`, `0022`–`0026`, and every migration `0029`–`0062`). Older migrations (`0001`–`0015`, plus the backfilled `0019`) do not have downs.
...
| `insurance_policies` | One **life** row per subscriber (`subscriber_id` PK); nullable. `status` ∈ `'active' \| 'inactive'` (TEXT — see D8). The extra products (health/funeral) live in **`subscriber_insurance_products`** (`0064`) — see below. (`0063` briefly made this table per-`(subscriber, product)`, but that broke the signup/employer upsert-by-subscriber_id RPCs; `0064` reverted it to the single-life-row contract and moved the new products out.) |
| `subscriber_insurance_products` | **NEW (`0064`)** — the non-life insurance products a subscriber holds. PK `(subscriber_id, product)`; `product` ∈ `'health' \| 'funeral'`; same `cover / premium_monthly / policy_start / renewal_date / status` columns as `insurance_policies`. RLS: subscriber self `SELECT`/`INSERT`/`UPDATE`, **plus agent `SELECT` (`sip_select_agent`, `0065`)** scoped to the agent's own subscribers (mirrors `insurance_policies_select_agent`) so the agent dashboard can list a subscriber's active policies. Written by `pay_insurance_premium` (`0064`) + the subscriber's own `update_self` RLS (renewals) + **the signup chain (`0065`) when a subscriber is onboarded with health/funeral selected**. |
...
**The new Singapore DB (`ilkhfnoyxlxwqadebnkp`, `ap-southeast-1`, cutover 2026-06-05) was rebuilt clean by replaying every migration `0001`–`0042` in order at the cutover** (via the Supabase MCP `apply_migration`); the `0043`–`0062` batch was then applied incrementally on top (per-file, the routine path). Its `supabase_migrations.schema_migrations` ledger therefore records all 62 — including the backfilled `0019_fix_metrics_rollup_app_role` — with sequential timestamps: **files and ledger are fully in sync, no drift.** This is a clean break from the retired Tokyo project, whose ledger had accumulated drift (kept below for history).

- **New DB = no drift.** Files `0001`–`0062` ↔ ledger `0001`–`0062`. Because the rebuild applied each file exactly once and in order, `db push` would now find nothing to re-attempt. The **routine** path is still explicit per-file apply (`mcp__supabase__apply_migration` / `psql -f <file>` against `SUPABASE_DB_URL`) + `scripts/seed-supabase.mjs` — and `supabase db reset` remains **local-only** (it drops + reseeds; never run it against live).
