...
| Table | Purpose |
|---|---|
| `employers` | One row per B2B account (`emp-001`). `name`, `sector`, `registration_no`, `contact_*`, `district`, `payroll_cadence`, `default_contribution_config JSONB` (`{ mode, employerPct/matchPct, maxContribution, insuranceEnabled, groupCoverAmount }` — the single company-wide funding template applied to every tagged member). |
