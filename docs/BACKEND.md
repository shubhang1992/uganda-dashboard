...
| `0048_remove_employer_member.sql` (+ `.down.sql`) | ~60 | **Employer "Remove from company" RPC.** Adds ONE SECURITY DEFINER RPC, `remove_employer_member(p_subscriber_id text) → jsonb`, gated on `app_role = 'employer'` + scoped to the `employerId` claim (the WHERE also pins `employer_id` so one employer can't un-link another's member). UN-LINKS a member by setting `subscribers.employer_id = NULL` — it deliberately does NOT touch `is_active`, so the person's subscriber account stays active and they continue as an individual saver. House grant pattern (`REVOKE … PUBLIC, anon; GRANT … authenticated, service_role`). The mock path keys an in-session removed-id set instead. **NOT yet applied to live — repo-only, pending approval** (the roster's Remove action works in mock mode now; the Supabase-backed demo needs this RPC). (Migrations `0043`–`0047` are also not yet catalogued in this table.) |
