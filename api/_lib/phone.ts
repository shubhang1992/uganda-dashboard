...
const VALID_PREFIXES = new Set(['70', '71', '74', '75', '76', '77', '78']);

export function parseUGPhoneLocal(raw: unknown): string {
  if (raw == null) return '';
...
export function isValidUGPhone(raw: unknown): boolean {
  const local = parseUGPhoneLocal(raw);
  if (local.length !== 9) return false;
  return VALID_PREFIXES.has(local.slice(0, 2));
}
