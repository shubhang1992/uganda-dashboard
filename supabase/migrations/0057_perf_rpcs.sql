-- =============================================================================
-- Universal Pensions Uganda — 0057: backend perf RPCs (audit §5b.1 / 5b.2 / 5b.3)
-- =============================================================================
-- Three read-only rollup RPCs rewritten for fewer buffers / better scaling, with
-- BYTE-FOR-BYTE identical output contracts (same signature, same jsonb keys, same
-- ordering) so the mocked frontend tests and every consumer stay green:
--
--   * get_platform_overview()        — §5b.2: the 6 separate subscriber COUNT
--       subqueries + the 2 transaction SUM subqueries collapse into ONE scan each
--       via count(*)/sum(...) FILTER. Was 13,401 buffers (6 full subscribers
--       scans + 2 transactions scans); now one subscribers pass + one
--       transactions pass. Same 13 output keys, same values.
--   * get_all_employers_metrics()    — §5b.3: was O(employers) correlated scalar
--       subqueries (6 per employer). Rewritten SET-BASED: each source
--       (subscribers / subscriber_balances / transactions / insurance_policies)
--       is pre-aggregated by employer_id in its own CTE (one row per employer,
--       no fan-out), then LEFT JOINed once onto employers. Same 12 keys, same
--       `ORDER BY name` array order, same numbers.
--   * get_entity_metrics_rollup()    — §5b.1 (search_path half): re-emitted with
--       the body UNCHANGED, only pinning SET search_path = public, pg_temp (was
--       bare `public`, missing pg_temp — house-convention drift, B2 §1b.1). No
--       logic / output change.
--
-- CONVENTIONS (mirroring 0049 / 0050):
--   * LANGUAGE plpgsql; STABLE; SECURITY DEFINER; SET search_path = public, pg_temp
--   * REVOKE ALL FROM PUBLIC; GRANT EXECUTE TO authenticated (same grantees the
--     prior definitions used).
--   * Admin RPCs gate on (SELECT auth.jwt()) ->> 'app_role' = 'admin' (CLAUDE.md
--     §5.7 — read 'app_role', NEVER 'role'; (SELECT ...) wrapper = 0008 initplan
--     opt). get_entity_metrics_rollup keeps its own multi-role gate verbatim.
--
-- Forward-only; reversible via 0057_perf_rpcs.down.sql (which restores the prior,
-- correct-but-slower bodies — for get_entity_metrics_rollup the .down restores
-- the HARDENED public, pg_temp definition, never the vulnerable bare-public one).
-- =============================================================================

-- =============================================================================
-- 1) get_platform_overview() — single-pass FILTER aggregates (§5b.2)
-- =============================================================================
-- Identical 13 keys / values to 0050; only the SQL shape changes:
--   * the 6 subscriber counts become ONE count(*) FILTER pass over subscribers
--   * the 2 transaction sums become ONE sum(...) FILTER pass over transactions
-- Distributor / employer / branch / agent counts and the AUM sum stay as their
-- own single-table scalar subqueries (one cheap pass each — nothing to fold).
...
BEGIN
  ...
  WITH subs AS (
    -- ONE pass over subscribers — all six headcount partitions via FILTER.
    SELECT
  ...
  txn AS (
    -- ONE pass over transactions — both money totals via FILTER.
    SELECT
...
-- =============================================================================
-- 2) get_all_employers_metrics() — set-based, pre-aggregated joins (§5b.3)
-- =============================================================================
-- Was 6 correlated scalar subqueries PER employer (O(employers)). Now each source
-- is pre-aggregated by employer_id in its own CTE — one row per employer, so the
-- final LEFT JOINs onto employers do NOT fan out — then assembled once. Identical
-- 12 keys, identical `ORDER BY name` array order, identical numbers.
...
BEGIN
  ...
  WITH subs AS (
    -- Headcount + active count, one pass over subscribers grouped by employer.
    SELECT
      s.employer_id,
      count(*)                            AS headcount,
  ...
  bal AS (
    -- Total balance across an employer's subscribers, one pass.
    SELECT
      s.employer_id,
  ...
  txn AS (
    -- Contribution totals (all + employer-sourced) in one pass over transactions.
    SELECT
      s.employer_id,
  ...
  ins AS (
    -- Active insurance policy count per employer, one pass.
    SELECT
      s.employer_id,
...
-- =============================================================================
-- 3) get_entity_metrics_rollup() — re-emit with pinned search_path (§5b.1)
-- =============================================================================
-- Body UNCHANGED from 0020. The ONLY difference vs the live definition is the
-- search_path: `public` -> `public, pg_temp` (house-convention pin; B2 §1b.1).
-- Output contract (signature TEXT, TEXT[] -> jsonb; every jsonb key; all numbers)
-- is byte-for-byte identical so every map/report consumer + mocked test is intact.
