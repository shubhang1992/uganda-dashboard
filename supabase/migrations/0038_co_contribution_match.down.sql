-- =============================================================================
-- DOWN — 0038_co_contribution_match.down.sql (manual-only rollback)
-- =============================================================================
-- Reverses 0038: restores the ORIGINAL salary-based co-contribution body of
-- submit_contribution_run from 0035 (employer pays employerPct% of salary AND
-- employee pays employeePct% of salary — two independent % of salary). NOT part
-- of the forward-only chain; for manual/emergency use:
--
--   psql "$SUPABASE_DB_URL" -f supabase/migrations/0038_co_contribution_match.down.sql
--
-- Re-CREATE-OR-REPLACE keeps the signature, SECURITY DEFINER, search_path, the
-- employerId JWT-claim gate, the nonce idempotency ledger, totals accumulation,
-- and the inline balance/units bump byte-identical to 0035 — only the
-- per-employee co-contribution branch reverts to the salary-based math. Run
-- BEFORE 0035's down (this body references the same employer tables).
-- =============================================================================
...
-- =============================================================================
-- End of 0038_co_contribution_match.down.sql
-- =============================================================================
