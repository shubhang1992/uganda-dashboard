...
import { menuIcon, logoutIcon } from '../desktop/icons';
import { DESKTOP_PRIMARY_NAV, DESKTOP_SECONDARY_NAV } from './employerNav';
import styles from './EmployerSideNavDesktop.module.css';
...
/**
 * EmployerSideNavDesktop — the labelled white rail for the employer DESKTOP
 * shell (>=1024px). Mirrors AgentSideNavDesktop: white skin, real-colour logo,
 * a collapse control (driven by `collapsed`/`onToggleCollapse` lifted to
 * EmployerDesktopShell so the grid column reflows), NavLink active states, and a
 * support badge (open-ticket count). Nav metadata comes from employerNav so the
 * rail never drifts from the route table.
 */
export default function EmployerSideNavDesktop({ collapsed = false, onToggleCollapse }) {
