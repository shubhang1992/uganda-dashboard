...
import { sparkIcon, closeIcon, sendIcon } from '../desktop/icons';
import styles from './EmployerCopilotPanel.module.css';

/**
 * EmployerCopilotPanel — the right-side AI chat panel for the employer DESKTOP
 * shell. Mirrors AgentCopilotPanel structurally (third grid column, stays
 * mounted, `inert` when closed, Esc-to-close), but answers come from the ASYNC
 * getEmployerChatResponse over the employer's own RLS-scoped figures — the same
 * truthful, mock copilot the mobile Overview hero uses (shared context builder).
 */
export default function EmployerCopilotPanel({ open, onClose, panelId }) {
  ...
  function ask(text) {
    ...
    getEmployerChatResponse(trimmed, ctx)
      .then((reply) => {
        // Brief settle so the typing indicator reads as "thinking" (mirrors the
        // mobile hero's 900ms beat, trimmed for the on-demand desktop panel).
      ...
      <div className={styles.inner}>
          ...
          {empty ? (
            <div className={styles.welcome}>
              ...
              <p className={styles.welcomeText}>
                Ask me anything about your staff, contribution runs, company funding, group
                insurance, or who&apos;s pending KYC.
              </p>
        ...
        <form className={styles.composer} onSubmit={handleSubmit}>
          <input
            ...
            placeholder="Ask about staff, runs, insurance…"
